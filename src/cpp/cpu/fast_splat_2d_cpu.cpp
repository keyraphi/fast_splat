...
#pragma omp atomic
      image[y_top * width * 3 + static_cast<size_t>(x_left) * 3 + 0] += weight_top_left * red;
#pragma omp atomic
      image[y_top * width * 3 + static_cast<size_t>(x_left) * 3 + 1] += weight_top_left * green;
#pragma omp atomic
      image[y_top * width * 3 + static_cast<size_t>(x_left) * 3 + 2] += weight_top_left * blue;
    }
...
#pragma omp atomic
      image[y_bottom * width * 3 + static_cast<size_t>(x_left) * 3 + 0] += weight_bottom_left * red;
#pragma omp atomic
      image[y_bottom * width * 3 + static_cast<size_t>(x_left) * 3 + 1] +=
          weight_bottom_left * green;
#pragma omp atomic
      image[y_bottom * width * 3 + static_cast<size_t>(x_left) * 3 + 2] += weight_bottom_left * blue;
    }
  }
...
#pragma omp atomic
      image[y_top * width * 3 + static_cast<size_t>(x_right) * 3 + 0] += weight_top_right * red;
#pragma omp atomic
      image[y_top * width * 3 + static_cast<size_t>(x_right) * 3 + 1] += weight_top_right * green;
#pragma omp atomic
      image[y_top * width * 3 + static_cast<size_t>(x_right) * 3 + 2] += weight_top_right * blue;
    }
    if (y_bottom >= 0 && y_bottom < static_cast<int>(height)) {
      const float weight_bottom = y - static_cast<float>(y_top);
      const float weight_bottom_right = weight_right * weight_bottom;
#pragma omp atomic
      image[y_bottom * width * 3 + static_cast<size_t>(x_right) * 3 + 0] +=
          weight_bottom_right * red;
#pragma omp atomic
      image[y_bottom * width * 3 + static_cast<size_t>(x_right) * 3 + 1] +=
          weight_bottom_right * green;
#pragma omp atomic
      image[y_bottom * width * 3 + static_cast<size_t>(x_right) * 3 + 2] +=
  ...
  for (size_t i = 0; i < patch_count; ++i) {
    ...
    for (size_t y = 0; y < patch_height; y++) {
      for (size_t x = 0; x < patch_width; x++) {
        const float red = patch[y * patch_width * 3 + x * 3 + 0];
        const float green = patch[y * patch_width * 3 + x * 3 + 1];
        const float blue = patch[y * patch_width * 3 + x * 3 + 2];
