...
def main():
    ## Creating the inputs ###############################
    # Creating the splat target image [rgb height, width]
    target = torch.zeros([3, 123, 321])

    # Create patches to splat into the target image
    # The patches all have to have the same size [n_patches, rgb, patch_height, patch_width]
    # (patch_height can be different to patch_width)
    patches = torch.rand([100, 3, 42, 42])

    # Specify position of each patch (float)
    # positions[0, :] are the X-coordinates
    # positions[1, :] are the Y-coordinates
    positions = torch.rand([2, 100])
    ...
    new_patches = torch.rand([3, 500, 42, 42])
    new_patchespositions = torch.rand([2, 100])
    ...
    if not torch.cuda.is_available():
        ...
        return
    target = torch.zeros([3, 123, 321], device="cuda:0")

    # Create patches to splat into the target image
    # The patches all have to have the same size [n_patches, patch_height, patch_width, rgb]
    # (patch_height can be different to patch_width)
    patches = torch.rand([100, 3, 42, 42], device="cuda:0")

    # Specify position of each patch (float)
    # positions[:, 0] are the X-coordinates
    # positions[:, 1] are the Y-coordinates
    positions = torch.rand([2, 100], device="cuda:0")
    positions[0, :] *= 321  # x-coordinate between 0 and 321
    positions[1, :] *= 123  # y-coordinate betewen 0 and 123
